...
app.post('/api/ttm', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'INSERT INTO ttm (ttm_name, ttm_creator_email, ttm_delegation) VALUES ($1,$2,$3) RETURNING *',
      [ttm_name, ttm_creator_email ?? null, ttm_delegation ?? null]
    );
...
app.post('/api/tasks', async (req, res) => {
  ...
  try {
    ...
    const orderRes = await client.query(
      'SELECT COALESCE(MAX(task_order), 0) + 1 AS n FROM tasks WHERE ttm_id = $1',
      [ttm_id]
    );
    const countRes = await client.query(
      'SELECT COUNT(*) AS c FROM tasks WHERE ttm_id = $1',
      [ttm_id]
    );
    const taskOrder = orderRes.rows[0].n;
    const taskNum   = parseInt(countRes.rows[0].c, 10) + 1;
    const taskRes   = await client.query(
      'INSERT INTO tasks (ttm_id, task_number, task_name, task_order) VALUES ($1,$2,$3,$4) RETURNING *',
      [ttm_id, String(taskNum), '', taskOrder]
    );
...
app.post('/api/ttm-resources', async (req, res) => {
  ...
  try {
    const existRes = await pool.query(
      'SELECT 1 FROM ttm_resources WHERE ttm_id = $1 AND resource_id = $2',
      [ttm_id, resource_id]
    );
    if (existRes.rows.length > 0) return res.status(409).json({ error: 'Resource already added to this TTM' });
    const orderRes = await pool.query(
      'SELECT COALESCE(MAX(resource_order), 0) + 1 AS n FROM ttm_resources WHERE ttm_id = $1',
      [ttm_id]
    );
    await pool.query(
      'INSERT INTO ttm_resources (ttm_id, resource_id, resource_order) VALUES ($1,$2,$3)',
      [ttm_id, resource_id, orderRes.rows[0].n]
    );
...
app.put('/api/ttm', async (req, res) => {
  ...
  try {
    if (type === 'task') {
      ...
      if (!task_id || !['task_number', 'task_name'].includes(field))
      ...
      if (!subtask_id || !['subtask_number', 'subtask_name'].includes(field))
        return res.status(400).json({ error: 'Invalid subtask update payload' });
      ...
      if (!ttm_id || !task_id || !subtask_id || !resource_id)
        return res.status(400).json({ error: 'Missing required fields for hours update' });
      const hours = (value === '' || value === null || value === undefined)
        ? null : parseFloat(value);
      const existing = await pool.query(
        'SELECT ttm_entry_id FROM ttm_entries WHERE ttm_id=$1 AND task_id=$2 AND subtask_id=$3 AND resource_id=$4',
        [ttm_id, task_id, subtask_id, resource_id]
      );
      if (existing.rows.length > 0) {
        await pool.query(
          'UPDATE ttm_entries SET ttm_entry_hours = $1 WHERE ttm_entry_id = $2',
          [hours, existing.rows[0].ttm_entry_id]
        );
      } else {
        await pool.query(
          'INSERT INTO ttm_entries (ttm_id, task_id, subtask_id, resource_id, ttm_entry_hours) VALUES ($1,$2,$3,$4,$5)',
          [ttm_id, task_id, subtask_id, resource_id, hours]
        );
      }
...
app.delete('/api/tasks/:task_id', async (req, res) => {
  ...
  try {
    await client.query('BEGIN');
    await client.query(
      'DELETE FROM ttm_entries WHERE subtask_id IN (SELECT subtask_id FROM subtasks WHERE task_id = $1)',
      [task_id]
    );
...
app.delete('/api/ttm-resources/:ttm_id/:resource_id', async (req, res) => {
  ...
  try {
    await client.query('BEGIN');
    await client.query(
      'DELETE FROM ttm_entries WHERE ttm_id = $1 AND resource_id = $2',
      [ttm_id, resource_id]
    );
    await client.query(
      'DELETE FROM ttm_resources WHERE ttm_id = $1 AND resource_id = $2',
      [ttm_id, resource_id]
    );
    await client.query('COMMIT');
    res.json({ ok: true });
...
app.put('/api/ttm/:ttm_id/task-order', async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < taskIds.length; i++) {
      await client.query(
        'UPDATE tasks SET task_order = $1 WHERE task_id = $2 AND ttm_id = $3',
        [i + 1, taskIds[i], ttm_id]
      );
    }
    await client.query('COMMIT');
    res.json({ ok: true });
...
app.put('/api/ttm/:ttm_id/resource-order', async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < resourceIds.length; i++) {
      await client.query(
        'UPDATE ttm_resources SET resource_order = $1 WHERE resource_id = $2 AND ttm_id = $3',
        [i + 1, resourceIds[i], ttm_id]
      );
    }
    await client.query('COMMIT');
    res.json({ ok: true });
