...
app.post('/api/ttm', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.request()
      ...
      .input('ttm_delegation',     sql.NVarChar(sql.MAX), ttm_delegation    ?? null)
      .query(`
  ...
  try {
    ...
    const [tasksRes, subsRes, resourcesRes] = await Promise.all([
      pool.request()
        ...
        .query('SELECT * FROM tasks WHERE ttm_id = @ttm_id ORDER BY task_order'),
      pool.request()
        .input('ttm_id', sql.Int, ttm_id)
        .query(`
          SELECT s.*
  ...
  try {
    ...
    const [subsRes, resRes] = await Promise.all([
      pool.request()
        ...
        .input('subLimit',  sql.Int, subTo - subFrom + 1)
        .query(`
          SELECT s.subtask_id
          FROM subtasks s
          JOIN tasks t ON t.task_id = s.task_id
      ...
      pool.request()
        ...
        .input('resLimit',  sql.Int, resTo - resFrom + 1)
        .query(`
          SELECT resource_id
          FROM ttm_resources
          WHERE ttm_id = @ttm_id
...
app.post('/api/tasks', async (req, res) => {
  ...
  try {
    ...
    const countRes = await new sql.Request(transaction)
      .input('ttm_id', sql.Int, ttm_id)
      .query('SELECT COUNT(*) AS c FROM tasks WHERE ttm_id = @ttm_id');
    ...
    const taskRes = await new sql.Request(transaction)
      ...
      .input('task_order',  sql.Int,           taskOrder)
      .query(`
...
app.delete('/api/subtasks/:subtask_id', async (req, res) => {
  ...
  try {
    ...
    await new sql.Request(transaction)
      .input('subtask_id', sql.Int, subtask_id)
      .query('DELETE FROM ttm_entries WHERE subtask_id = @subtask_id');
    await new sql.Request(transaction)
      .input('subtask_id', sql.Int, subtask_id)
