...
function TaskRow({ task, style, onEditTaskField, onDeleteTask }) {
	return (
		<div style={style} className="lp-row lp-row-task">
			<EditableCell value={task.task_number} className="lp-cell-num lp-cell-task" onSave={(v) => onEditTaskField(task.task_id, 'task_number', v)} />
			<div className="lp-cell-name-wrap">
				<EditableCell value={task.task_name} className="lp-cell-name lp-cell-task" onSave={(v) => onEditTaskField(task.task_id, 'task_name', v)} />
				<button className="lp-del-btn" title="Delete task" onClick={() => onDeleteTask(task.task_id, task.task_name)}>
					×
				</button>
			</div>
		</div>
...
const LeftPanel = memo(function LeftPanel({ rows, scrollTop, onEditTaskField, onEditSubtaskField, onDeleteTask, onDeleteSubtask }) {
	...
	const endIdx = Math.min(rows.length - 1, Math.ceil((scrollTop + localHeight) / ROW_HEIGHT) + OVERSCAN_ROWS);

	const visibleRows = rows.slice(startIdx, endIdx + 1);

	return (
		<div ref={outerRef} className="lp-outer" style={{ position: 'relative', height: '100%', overflow: 'hidden', width: TASK_NUM_W + TASK_NAME_W }}>
