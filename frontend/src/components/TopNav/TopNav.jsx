...
export default function TopNav({ currentTtm, onHome, onAddTask, onAddSubtasks, onAddResource, onReorder }) {
	...
	return (
		<nav className="topnav">
			...
			{viewing && <span className="topnav-ttm-name">{currentTtm.ttm_name}</span>}

			<div className="topnav-spacer" />

			{viewing && (
				<div className="topnav-actions">
					<button className="topnav-btn topnav-btn-ghost" onClick={() => onReorder('tasks')} title="Reorder tasks">
						⇅ Tasks
					</button>
					<button className="topnav-btn topnav-btn-ghost" onClick={() => onReorder('resources')} title="Reorder resources">
						⇄ Resources
					</button>
			...
			{!viewing && <span className="topnav-tagline">Time Task Matrix</span>}
		</nav>
	);
}
