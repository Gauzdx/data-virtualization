			...
			<div className="hp-modal" onClick={(e) => e.stopPropagation()}>
				...
				<form onSubmit={submit} className="hp-form">
					<label className="hp-label">
						...
						<input className="hp-input" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Q2 2026 Planning" autoFocus />
					</label>
					<label className="hp-label">
						Creator Email
						<input className="hp-input" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@company.com" />
					</label>
					<label className="hp-label">
						Delegation
						<input className="hp-input" value={delegation} onChange={(e) => setDeleg(e.target.value)} placeholder="e.g. Engineering" />
					</label>
...
export default function TtmHomePage() {
	...
	const goToTtm = (ttm) => {
		navigate(`/ttm/${ttm.ttm_id}`, { state: { ttm_name: ttm.ttm_name } });
	};
	...
	return (
		<div className="hp-shell">
			<div className="hp-hero">
				<button className="hp-home-btn" onClick={() => navigate('/')} title="Back to apps">
					← Home
				</button>
				<div className="hp-hero-content">
					...
					<p className="hp-hero-sub">Time Task Matrix</p>
			...
			<div className="hp-content">
				...
				{!loading && ttms.length > 0 && (
					<div className="hp-grid">
						{ttms.map((ttm) => (
							<button key={ttm.ttm_id} className="hp-card" onClick={() => goToTtm(ttm)}>
								<div className="hp-card-accent" />
								<div className="hp-card-body">
									<span className="hp-card-id">#{ttm.ttm_id}</span>
									<h3 className="hp-card-name">{ttm.ttm_name}</h3>
									{ttm.ttm_delegation && <span className="hp-card-tag">{ttm.ttm_delegation}</span>}
									{ttm.ttm_creator_email && <p className="hp-card-email">{ttm.ttm_creator_email}</p>}
								</div>
			...
			{showCreate && <CreateTtmModal onClose={() => setShowCreate(false)} onCreate={handleCreated} />}
		</div>
	);
}
